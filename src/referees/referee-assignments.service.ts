...
} satisfies Prisma.match_refereesSelect;

type AssignmentRecord = Prisma.match_refereesGetPayload<{
...
export class RefereeAssignmentsService {
  ...
  async findAssignableMatches(managerId: bigint, refereeId: bigint) {
    await this.assertActiveReferee(this.prisma, refereeId);
    const manageableTournamentIds =
      await this.access.findManageableTournamentIds(managerId);
    const now = new Date();
    const matches = await this.prisma.matches.findMany({
      where: {
        tournament_id:
          manageableTournamentIds === null
            ? undefined
            : { in: manageableTournamentIds },
        tournament_team_registrations_matches_tournament_id_home_team_idTotournament_team_registrations:
          { tournaments: { phase: { in: [...ASSIGNABLE_TOURNAMENT_PHASES] } } },
  ...
  async findMatchAssignments(managerId: bigint, matchId: bigint) {
  ...
  async assign(managerId: bigint, matchId: bigint, dto: AssignRefereeDto) {
    ...
    await this.access.assertTournamentInPhases(
      tournamentId,
      ASSIGNABLE_TOURNAMENT_PHASES,
      'Los árbitros solo se pueden asignar durante Validación, Programado o En curso.',
    );
    ...
    try {
      await this.prisma.$transaction(
        async (transaction) => {
          ...
          await this.persistPendingAssignment(
            transaction,
            match,
            refereeId,
            dto.role,
  ...
  async respond(
    refereeId: bigint,
    matchId: bigint,
    dto: RespondRefereeAssignmentDto,
  ) {
      ...
      select: {
        ...
        matches: {
          select: {
            ...
            status: true,
            tournament_team_registrations_matches_tournament_id_home_team_idTotournament_team_registrations:
              { select: { tournaments: { select: { phase: true } } } },
          },
        },
      },
    });
    if (!current || current.assignment_status !== 'pending') {
      throw new NotFoundException(
        'No existe una asignación pendiente para responder en este partido.',
      );
    }
    if (
      !ASSIGNABLE_TOURNAMENT_PHASES.includes(
        current.matches
          .tournament_team_registrations_matches_tournament_id_home_team_idTotournament_team_registrations
          .tournaments.phase as never,
      )
    ) {
      throw new BadRequestException(
        'Esta asignación ya no se puede responder porque el torneo está cerrado.',
      );
    }
        ...
        async (transaction) => {
          ...
          if (changed.count !== 1) {
            throw new ConflictException(
              'La asignación ya fue respondida desde otra sesión.',
            );
          }
          ...
          await this.notifyTournamentManagers(
            transaction,
            current.tournament_id,
            refereeId,
            dto.status === 'accepted'
              ? 'Asignación arbitral aceptada'
              : 'Asignación arbitral rechazada',
            dto.status === 'accepted'
              ? 'El árbitro confirmó su participación en el partido.'
              : `El árbitro rechazó la asignación${dto.notes?.trim() ? `: ${dto.notes.trim()}` : '.'}`,
            matchId,
          );
        },
        { isolationLevel: Prisma.TransactionIsolationLevel.Serializable },
  ...
  async replace(
    managerId: bigint,
    matchId: bigint,
    currentRefereeId: bigint,
    dto: ReplaceRefereeDto,
  ) {
    const tournamentId = await this.access.assertCanManageMatch(
      managerId,
      matchId,
    ...
    await this.access.assertTournamentInPhases(
      tournamentId,
      ASSIGNABLE_TOURNAMENT_PHASES,
      'Los árbitros solo se pueden reemplazar durante Validación, Programado o En curso.',
    );
    const newRefereeId = BigInt(dto.newRefereeId);
    if (newRefereeId === currentRefereeId) {
      throw new BadRequestException(
        'Selecciona un árbitro diferente para realizar el reemplazo.',
      );
    }
        ...
        async (transaction) => {
          ...
          if (
            ...
            !ACTIVE_ASSIGNMENT_STATUSES.includes(
              current.assignment_status as (typeof ACTIVE_ASSIGNMENT_STATUSES)[number],
            )
          ) {
            throw new NotFoundException(
              'La asignación que deseas reemplazar ya no está activa.',
            );
          }
          const match = await this.findAssignableMatch(transaction, matchId);
          ...
          await this.assertRoleIsAvailable(
            ...
            match.id,
            newRefereeId,
            current.referee_role as 'main' | 'assistant',
            currentRefereeId,
          );
  ...
  private async findAssignment(matchId: bigint, refereeId: bigint) {
    ...
    if (!assignment) {
      throw new NotFoundException('La asignación arbitral no existe.');
    }
    return this.toResponse(assignment);
  }

  private async findAssignableMatch(client: DatabaseClient, matchId: bigint) {
    ...
    const tournament =
      match
        .tournament_team_registrations_matches_tournament_id_home_team_idTotournament_team_registrations
        .tournaments;
    if (!ASSIGNABLE_TOURNAMENT_PHASES.includes(tournament.phase as never)) {
      throw new BadRequestException(
        'El torneo no permite gestionar árbitros en su fase actual.',
      );
    }
    if (['played', 'cancelled'].includes(match.status)) {
      throw new BadRequestException(
        'No se pueden asignar árbitros a un partido finalizado o cancelado.',
      );
    }
    if (!match.match_date) {
      throw new BadRequestException(
        'Define la fecha y la hora del partido antes de asignar árbitros.',
      );
    }
    return { ...match, tournaments: tournament };
  }

  private async assertActiveReferee(
    ...
    if (!referee) {
      throw new BadRequestException(
        'El usuario seleccionado no es un árbitro activo.',
      );
    }
  }

  private async assertCanAttend(
    client: DatabaseClient,
    refereeId: bigint,
    matchId: bigint,
    startsAt: Date | null,
    ...
    if (conflict) {
      throw new ConflictException(
        `El árbitro ya tiene una asignación que se cruza con este horario (partido ${conflict.match_id.toString()}).`,
      );
    }
  }
  ...
  private async assertRoleIsAvailable(
    ...
    if (duplicate) {
      throw new ConflictException(
        'Este árbitro ya tiene una asignación activa en el partido.',
      );
    }
    if (role !== 'main') return;
    ...
    if (main) {
      throw new ConflictException(
        'El partido ya tiene un árbitro principal pendiente o confirmado.',
      );
    }
  }

  private async persistPendingAssignment(
    ...
    match: {
      ...
      venue: string | null;
      tournaments: { name: string; phase: string };
    },
    refereeId: bigint,
    role: 'main' | 'assistant',
    managerId: bigint,
    replacedRefereeId?: bigint,
    reason?: string,
  ): Promise<void> {
  ...
  private async notifyTournamentManagers(
    transaction: Prisma.TransactionClient,
    tournamentId: bigint,
    actorId: bigint,
    title: string,
    ...
    const recipients = new Set<bigint>([
      ...
      ...tournament.associations.association_administrators.map(
        ({ user_id }) => user_id,
      ),
  ...
  private toResponse(assignment: AssignmentRecord) {
  ...
  private rethrowAssignmentConflict(error: unknown): never {
    ...
    if (
      ...
      throw new ConflictException(
        'La asignación cambió al mismo tiempo desde otra sesión. Actualiza la información e inténtalo de nuevo.',
      );
    }
    throw error;
  }
