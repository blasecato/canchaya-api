...
type RefereeMatchRecord = Prisma.match_refereesGetPayload<{
  select: typeof refereeMatchSelect;
}>;
...
export class RefereesService {
  ...
  async findAll(search?: string) {
    ...
    const directedByReferee = new Map(
      directedCounts.map((item) => [
        item.referee_id.toString(),
        item._count._all,
      ]),
    );
    const upcomingByReferee = new Map(
      upcomingCounts.map((item) => [
        item.referee_id.toString(),
        item._count._all,
      ]),
    );
  ...
  async findMatches(
    ...
    query: ListRefereeMatchesQueryDto,
  ) {
    ...
    if (id !== requestingUserId && !isManager) {
      throw new ForbiddenException(
        'Como árbitro solamente puedes consultar tus propios partidos.',
      );
    }
    ...
    if (!referee) {
      throw new NotFoundException(
        `El árbitro con ID ${id.toString()} no existe.`,
      );
    }
    const now = new Date();
    const matchWhere: Prisma.matchesWhereInput =
      query.kind === 'past'
        ? {
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.notifications.create({
        data: {
          ...
          message:
            'Ahora tienes el rol de jugador. Tus asignaciones arbitrales futuras fueron canceladas; tu historial de partidos se conserva.',
  ...
  private async findRoleCodes(userId: bigint) {
    const roles = await this.prisma.user_roles.findMany({
