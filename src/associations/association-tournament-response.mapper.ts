...
} satisfies Prisma.tournament_sponsorsSelect;

export type TournamentSponsorResponseRecord =
  Prisma.tournament_sponsorsGetPayload<{
    select: typeof tournamentSponsorResponseSelect;
  }>;

export const associationTournamentResponseSelect = {
  id: true,
  association_id: true,
  name: true,
  description: true,
...
export function toAssociationTournamentResponse(
  tournament: AssociationTournamentResponseRecord,
): AssociationTournamentResponseDto {
  return {
    ...
    // PostgreSQL garantiza estos dominios mediante restricciones CHECK.
    phase: tournament.phase as AssociationTournamentResponseDto['phase'],
    status: tournament.status as AssociationTournamentResponseDto['status'],
    createdAt: tournament.created_at.toISOString(),
    updatedAt: tournament.updated_at.toISOString(),
  };
}

export function toTournamentSponsorResponse(
  relation: TournamentSponsorResponseRecord,
): AssociationTournamentResponseDto['sponsors'][number] {
  return {
    ...
    contributionType:
      relation.contribution_type as AssociationTournamentResponseDto['sponsors'][number]['contributionType'],
    ...
    status:
      relation.status as AssociationTournamentResponseDto['sponsors'][number]['status'],
  };
}
...
function toNullableDateOnly(value: Date | null): string | null {
