...
exports.renderPasswordResetCodeEmail = renderPasswordResetCodeEmail;
const NAVY = '#0a1c3d';
...
const escapeHtml = (value) => value
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
const toDialLink = (phone) => phone.replace(/[^\d+]/g, '');
const toWhatsAppLink = (phone) => phone.replace(/\D/g, '');
const renderCodeBoxes = (code) => Array.from(code)
    .map((digit) => `<td align="center" valign="middle" width="46" bgcolor="${NAVY_DEEP}" style="width:46px;height:58px;border:2px solid ${GOLD};border-radius:10px;font-family:'Trebuchet MS',Arial,sans-serif;font-size:30px;font-weight:bold;color:${GOLD};">${escapeHtml(digit)}</td>
       <td width="8" style="width:8px;font-size:0;line-height:0;">&nbsp;</td>`)
    .join('');
const renderPitchStripes = () => Array.from({ length: 10 })
    .map((_unused, index) => `<td bgcolor="${index % 2 === 0 ? GRASS : '#175c31'}" style="height:6px;font-size:0;line-height:0;">&nbsp;</td>`)
    .join('');
function renderPasswordResetCodeEmail(data) {
...
//# sourceMappingURL=password-reset-code.template.js.map