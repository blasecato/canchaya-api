...
const actionSelect = {
    ...
    matches: {
        select: {
            ...
            home_score: true,
            away_score: true,
            tournament_team_registrations_matches_tournament_id_home_team_idTotournament_team_registrations: { select: { teams: { select: { id: true, name: true } } } },
...
let DisciplinaryActionsService = class DisciplinaryActionsService {
    ...
    async create(requestingUserId, dto) {
        ...
        if (!dto.matchId) {
            throw new common_1.BadRequestException('El informe arbitral debe estar asociado a un partido.');
        }
        ...
        await this.access.assertTournamentInPhases(tournamentId, ['in_progress'], 'Los informes arbitrales solo se pueden registrar mientras el torneo está En curso.');
        ...
        const actionId = await this.prisma.$transaction(async (transaction) => {
            ...
            await this.createEvent(transaction, action.id, requestingUserId, 'reported', dto.reason.trim(), { cardType: dto.cardType ?? 'none', matchId: matchId.toString() });
            ...
            await this.notifyMany(transaction, await this.findSuperAdminIds(transaction), {
    ...
    async findReportOptions(requestingUserId, matchId) {
        await this.access.assertHasAnyRole(requestingUserId, ['REFEREE']);
        await this.assertAcceptedRefereeAssignment(requestingUserId, matchId);
    ...
    async findAll(requestingUserId, query) {
        ...
        const [total, items, reported, underReview, approved, dismissed, appealsPending,] = await this.prisma.$transaction([
        ...
        return {
            items: items.map((item) => this.toResponse(item, accumulation.get(this.accumulationKey(item)))),
            metrics: {
    ...
    async findOne(id, requestingUserId) {
        ...
        return this.toResponse(action, accumulation.get(this.accumulationKey(action)));
    }
    async startReview(id, requestingUserId) {
        ...
        if (current.decision_status !== 'reported') {
            throw new common_1.ConflictException('Este informe ya fue tomado para revisión o decidido.');
        }
        const now = new Date();
        await this.prisma.$transaction(async (transaction) => {
            ...
            if (result.count !== 1) {
                throw new common_1.ConflictException('Otro usuario ya tomó este informe para revisión.');
            }
            await this.createEvent(transaction, id, requestingUserId, 'review_started', 'El informe entró en revisión.');
            await this.notifyMany(transaction, [current.player_id], {
                type: 'discipline_review',
                title: 'Tu informe disciplinario está en revisión',
                message: 'Un superadministrador inició la revisión del informe arbitral.',
                entityType: 'disciplinary_action',
    ...
    async decide(id, requestingUserId, dto) {
        await this.access.assertHasAnyRole(requestingUserId, ['SUPER_ADMIN']);
        const current = await this.findExisting(id);
        if (!['reported', 'under_review'].includes(current.decision_status)) {
            throw new common_1.ConflictException('Este informe ya tiene una decisión definitiva.');
        }
        if (dto.decision === 'dismissed' &&
            (dto.suspension || dto.fine || dto.block)) {
            throw new common_1.BadRequestException('Un informe desestimado no puede generar sanciones.');
        }
        if (dto.suspension &&
            !dto.suspension.matchesCount &&
            !dto.suspension.startDate &&
            !dto.suspension.endDate) {
            throw new common_1.BadRequestException('Indica partidos o fechas para la suspensión.');
        }
        await this.access.assertTournamentInPhases(current.tournament_id, ['in_progress', 'finished'], 'La gestión disciplinaria de este torneo ya está cerrada.');
        const now = new Date();
        const appealDeadline = dto.decision === 'approved'
        ...
        await this.prisma.$transaction(async (transaction) => {
            ...
            if (changed.count !== 1) {
                throw new common_1.ConflictException('Otro usuario ya decidió este informe.');
            }
            await this.createEvent(transaction, id, requestingUserId, dto.decision, dto.notes.trim());
            if (dto.decision === 'approved') {
                const automaticSuspension = await this.resolveAutomaticSuspension(transaction, current);
                    ...
                    await transaction.suspensions.create({
                        data: {
                            ...
                            end_date: suspension.endDate
                                ...
                                : null,
                            reason: suspension.reason?.trim() || 'Suspensión disciplinaria.',
                            status: 'active',
                            created_by: requestingUserId,
                        },
                    });
                    await this.createEvent(transaction, id, requestingUserId, 'suspension_created', suspension.reason ?? 'Suspensión aplicada.', {
                ...
                if (dto.fine) {
                    await transaction.fines.create({
                        data: {
                            disciplinary_action_id: id,
                            amount: new client_1.Prisma.Decimal(dto.fine.amount),
                            due_date: dto.fine.dueDate ? new Date(dto.fine.dueDate) : null,
                            notes: dto.fine.notes?.trim() || null,
                            created_by: requestingUserId,
                        },
                    });
                    await this.createEvent(transaction, id, requestingUserId, 'fine_created', dto.fine.notes?.trim() || 'Multa aplicada.', { amount: dto.fine.amount, currencyCode: 'COP' });
                }
                if (dto.block) {
                    if (current.player_id === requestingUserId) {
                        throw new common_1.BadRequestException('No puedes bloquear tu propia cuenta.');
                    }
                    const blockedUntil = this.calculateBlockedUntil(now, dto.block.duration);
                    ...
                    await this.createEvent(transaction, id, requestingUserId, 'account_blocked', dto.block.reason.trim(), { blockedUntil: blockedUntil.toISOString() });
                }
            }
            const tournament = await transaction.tournaments.findUniqueOrThrow({
            ...
            const measureText = dto.decision === 'dismissed'
            ...
            await this.notifyMany(transaction, [current.player_id], {
                ...
                message: `${tournament.name}: ${measureText}${appealDeadline
    ...
    async appeal(id, requestingUserId, dto) {
        await this.access.assertHasAnyRole(requestingUserId, ['PLAYER']);
        const current = await this.findExisting(id);
        if (current.player_id !== requestingUserId) {
            throw new common_1.ForbiddenException('Solo el jugador afectado puede apelar esta decisión.');
        }
        if (current.decision_status !== 'approved' || !current.appeal_deadline) {
            throw new common_1.BadRequestException('Este caso no tiene una decisión sancionatoria apelable.');
        }
        if (current.appeal_deadline.getTime() < Date.now()) {
            throw new common_1.BadRequestException('El plazo de apelación ya finalizó.');
        }
        try {
            await this.prisma.$transaction(async (transaction) => {
                ...
                await this.createEvent(transaction, id, requestingUserId, 'appealed', dto.message.trim());
                await this.notifyMany(transaction, await this.findSuperAdminIds(transaction), {
            ...
            if (this.isUniqueConstraint(error)) {
                throw new common_1.ConflictException('Este caso ya tiene una apelación registrada.');
            }
            throw error;
        }
        return this.findOne(id, requestingUserId);
    }
    async resolveAppeal(id, requestingUserId, dto) {
        await this.access.assertHasAnyRole(requestingUserId, ['SUPER_ADMIN']);
        const current = await this.findExisting(id);
        const appeal = current.disciplinary_appeals;
        if (!appeal || appeal.status !== 'pending') {
            throw new common_1.BadRequestException('Este caso no tiene una apelación pendiente.');
        }
        const now = new Date();
        await this.prisma.$transaction(async (transaction) => {
            ...
            await this.createEvent(transaction, id, requestingUserId, dto.decision === 'accepted' ? 'appeal_accepted' : 'appeal_rejected', dto.notes.trim());
            await this.notifyMany(transaction, [current.player_id], {
                type: 'discipline_appeal',
                title: dto.decision === 'accepted'
    ...
    async updateCompliance(id, requestingUserId, dto) {
        await this.access.assertHasAnyRole(requestingUserId, ['SUPER_ADMIN']);
        const current = await this.findExisting(id);
        if (current.decision_status !== 'approved') {
            throw new common_1.BadRequestException('Solo se puede registrar cumplimiento en sanciones aprobadas.');
        }
        if (!dto.suspensionStatus && !dto.fineStatus && !dto.unblockAccount) {
            throw new common_1.BadRequestException('Indica al menos una medida por actualizar.');
        }
        if (dto.fineStatus === 'paid' && !dto.paymentReference?.trim()) {
            throw new common_1.BadRequestException('La referencia de pago es obligatoria para marcar una multa como pagada.');
        }
        const now = new Date();
        await this.prisma.$transaction(async (transaction) => {
            if (dto.suspensionStatus) {
                ...
                if (!result.count) {
                    throw new common_1.BadRequestException('No existe una suspensión activa en este caso.');
                }
                await this.createEvent(transaction, id, requestingUserId, dto.suspensionStatus === 'served'
                    ? 'suspension_served'
                    : 'suspension_revoked', dto.notes?.trim());
            }
            if (dto.fineStatus) {
                const result = await transaction.fines.updateMany({
                    ...
                    data: {
                        ...
                        payment_reference: dto.fineStatus === 'paid' ? dto.paymentReference?.trim() : null,
                        updated_at: now,
                    },
                });
                if (!result.count) {
                    throw new common_1.BadRequestException('No existe una multa pendiente en este caso.');
                }
                await this.createEvent(transaction, id, requestingUserId, dto.fineStatus === 'paid'
                    ? 'fine_paid'
                    : dto.fineStatus === 'waived'
                        ? 'fine_waived'
                        : 'fine_cancelled', dto.notes?.trim());
            }
            if (dto.unblockAccount) {
                ...
                if (!result.count) {
                    throw new common_1.BadRequestException('La cuenta no está bloqueada por este caso.');
                }
                await this.createEvent(transaction, id, requestingUserId, 'account_unblocked', dto.notes?.trim());
            }
            await this.notifyMany(transaction, [current.player_id], {
                type: 'discipline_compliance',
                title: 'Se actualizó el cumplimiento de tu sanción',
                message: dto.notes?.trim() || 'Una medida disciplinaria cambió de estado.',
                entityType: 'disciplinary_action',
                entityId: id.toString(),
                actionUrl: '/discipline',
    ...
    async update(id, requestingUserId, dto) {
        ...
        if (dto.decisionStatus !== undefined || dto.decisionNotes !== undefined) {
            throw new common_1.BadRequestException('Usa el endpoint de decisión para aprobar o desestimar el informe.');
        }
        const current = await this.findExisting(id);
        if (!['reported', 'under_review'].includes(current.decision_status)) {
            throw new common_1.BadRequestException('Un informe decidido ya no puede corregirse.');
        }
        ...
        if (!matchId) {
            throw new common_1.BadRequestException('El informe debe conservar un partido asociado.');
        }
        await this.assertValidSubject(tournamentId, matchId, teamId, playerId);
    ...
    async remove(id, requestingUserId) {
        ...
        const current = await this.findExisting(id);
        if (current.decision_status !== 'reported') {
            throw new common_1.BadRequestException('Solo se pueden eliminar informes que aún no estén en revisión.');
        }
    ...
    async findExisting(id) {
        ...
        if (!action) {
            throw new common_1.NotFoundException(`No se encontró el caso disciplinario con ID ${id.toString()}.`);
        }
        return action;
    }
    ...
    async assertCanViewAction(action, requestingUserId) {
        const roles = await this.access.findRoleCodes(requestingUserId);
        if (roles.has('SUPER_ADMIN') ||
        ...
        if (roles.has('ASSOCIATION_ADMIN')) {
            const ids = await this.access.findManageableTournamentIds(requestingUserId);
            if (ids?.some((value) => value === action.tournament_id))
                return;
        }
        throw new common_1.ForbiddenException('No tienes permisos para consultar este caso disciplinario.');
    }
    async assertAcceptedRefereeAssignment(refereeId, matchId) {
        ...
        if (!assignment || assignment.assignment_status !== 'accepted') {
            throw new common_1.ForbiddenException('Solo un árbitro con asignación aceptada puede informar este partido.');
        }
    }
    async assertValidSubject(tournamentId, matchId, teamId, playerId) {
        const match = await this.prisma.matches.findUnique({
            where: { id: matchId },
            select: {
                tournament_id: true,
        ...
        if (!roster || roster.registration_status !== 'approved') {
            throw new common_1.BadRequestException('El jugador debe estar aprobado en la plantilla del torneo.');
        }
    }
    async resolveAutomaticSuspension(transaction, action) {
        if (action.card_type === 'red' || action.card_type === 'double_yellow') {
            return {
                matchesCount: 1,
                reason: action.card_type === 'red'
                    ? 'Suspensión automática por tarjeta roja.'
                    : 'Suspensión automática por doble tarjeta amarilla.',
            };
        }
        if (action.card_type !== 'yellow')
            return null;
    ...
    async getAccumulationForActions(actions) {
        const keys = new Map();
        ...
        await Promise.all([...keys.entries()].map(async ([key, value]) => {
            ...
            for (const row of grouped) {
                ...
                if (row.card_type === 'red')
                    counts.redCards = row._count._all;
            }
            result.set(key, counts);
        }));
        return result;
    }
    accumulationKey(action) {
        return `${action.tournament_id.toString()}:${action.player_id.toString()}`;
    }
    toResponse(action, accumulation = {
        yellowCards: 0,
        doubleYellowCards: 0,
        redCards: 0,
    }) {
        ...
        return {
            ...
            reportedBy: this.toPerson(action.users_disciplinary_actions_reported_byTousers),
            ...
            appeal: action.disciplinary_appeals
                ? {
                    ...
                    reviewedAt: action.disciplinary_appeals.reviewed_at?.toISOString() ?? null,
            ...
            suspension: action.suspensions
                ? {
                    ...
                    startDate: action.suspensions.start_date?.toISOString().slice(0, 10) ?? null,
                    endDate: action.suspensions.end_date?.toISOString().slice(0, 10) ?? null,
            ...
            accumulation: {
                ...accumulation,
                nextAutomaticSuspensionAt: (Math.floor(accumulation.yellowCards / YELLOW_CARD_SUSPENSION_THRESHOLD) +
                    1) *
    ...
    async createEvent(transaction, actionId, actorId, eventType, message, metadata) {
    ...
    async findSuperAdminIds(database) {
    ...
    async notifyMany(transaction, userIds, data) {
        if (!userIds.length)
            return;
    ...
    buildMeasuresText(dto) {
        const measures = [];
        if (dto.suspension)
            measures.push('se aplicó una suspensión');
        if (dto.fine) {
            measures.push(`se aplicó una multa de $${dto.fine.amount.toLocaleString('es-CO')}`);
        }
        if (dto.block) {
